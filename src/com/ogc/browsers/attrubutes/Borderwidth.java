...
import com.ogc.dialog.EwpBroderwidthDialog;

public class Borderwidth extends QRAttribute{
	private QRWebPageEditorView fa;
	private String id;
	public Borderwidth(Element element) {
		super(element);
		setName("Border width");
		setProperty(".style.borderWidth");
		if(hasStyleAttribute("border-width")){
			setAttribute(getStyleAttribute("border-width"));
		}else{
			setAttribute("");
		}
	}
	...
	public void onTouch(QRWebPageEditorView fa, String id) {
		...
		if(((String)getAttribute()).equals("")){
			new EwpBroderwidthDialog(fa.getContext(),-1,this);
		}else{
			new EwpBroderwidthDialog(fa.getContext(),Integer.valueOf(((String)getAttribute()).replace("px", "").trim()),this);
